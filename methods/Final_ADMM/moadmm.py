...
class MultiObjectiveADMM:
    ...
    def initialize_x(self):
        # x0 = np.random.uniform(self.problem.lb, self.problem.ub, self.n)
        ...
        # x0 = np.array([-2.65735412, -4.06196869])
    ...
    def line_search(self, x_k, z_k, d_x, d_z):
        ...
        while t > 1e-10:
            ...
            if bnds is not None:
                ...
                lbs = np.array(lbs)
                ubs = np.array(ubs)
            bounds_check = np.all(x_new >= lbs) and np.all(x_new <= ubs)
            # bounds_check = True
            if np.all(f_new <= f_curr + self.beta * t * descent) and bounds_check:
                return t
            t *= 0.5
    ...
    def solve(self):
        ...
        # print(f"Initial x: {x}, Initial z: {z}")
        ...
        for k in range(self.max_outer):
            ...
            global_res = np.linalg.norm(x_new - x)
        ...
        return {'x':x, 'z':z, 'f_val':self.problem.evaluate_f(x), 'g_val':self.problem.evaluate_g(z), 'f':self.problem.evaluate(x, z), 'iterations':k,
        'residual':global_res, 'runtime':time()-start_time}